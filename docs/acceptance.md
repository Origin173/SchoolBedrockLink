...
1. 所有页面由同一个渲染层（`HtmlPages`）生成，本次只改样式：表单 action、字段名、按钮文案、“错误编号”文案和跳转时序完全未变，其余验收步骤不需要改写。
2. 自查项：320–1600px 宽度下无横向滚动；输入框聚焦为蓝色描边；已选角色的卡片为深色描边；成功页为蓝色对勾，错误页为红色感叹号。
3. 样式是 CSP 允许的内联 `<style>`，不加载任何外部字体或图片（字体回落到系统字体）；`:has()` 与 `mask-image` 只影响选中态高亮与顶部渐变，旧浏览器降级后功能不受影响。
